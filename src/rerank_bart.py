...
info_to_save = "base_Hsize_4_beam_search"
model_scale = "base"
...
tokenizer = BartTokenizer.from_pretrained(f"{TOKENIZER_OUTPUT_DIR}/bart_{model_scale}/")
bert_tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")

generator_model = BartForConditionalGeneration.from_pretrained(
    f"{MODELS_OUTPUT_DIR}/bart_{model_scale}/final/model", return_dict_in_generate=True
)
...
with torch.no_grad():
    ...
    with (open(f"{PREDS_OUTPUT_DIR}/wrongs/rerank_bart/{DATA_TYPE_COMBINATION['test']}_wrongs_rerank_bart_{now}_{info_to_save}.csv", "w+") as f1,
        open(f"{PREDS_OUTPUT_DIR}/rights/rerank_bart/{DATA_TYPE_COMBINATION['test']}_rights_rerank_bart_{now}_{info_to_save}.csv", "w+") as f2):
        for idx, row in test_df.iterrows():
            ...
            for rank, (sentence_score, sequence) in enumerate(sorted_res):
                ...
                candidate = (
                    ...
                    .replace("<s>", "")
                    .replace("</s>", "").replace("<pad>", "").replace("<tutor> ", "").replace("  ", " ").strip()
                )
                if rank == 0:
                    lm_rank_1 = candidate
            ...
            if pred_text == "":
                ...
                f1.write("\n")
                preds_labels.append(int(pred))
                rprint(f"[red]{round(count_wrong/len_, ndigits=3)*100}% lines with wrong labels")
...
preds_file_path = f"{PREDS_OUTPUT_DIR}/rerank_bart/{DATA_TYPE_COMBINATION['test']}_predictions_{now}_{info_to_save}.txt"
